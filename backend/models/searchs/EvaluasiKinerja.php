...
use backend\models\EvaluasiKinerja as EvaluasiKinerjaModel;
 ...
 */
class EvaluasiKinerja extends EvaluasiKinerjaModel
{
    /**
     * {@inheritdoc}
     */
    ...
    public function search($params)
    {
        ...
        $query = KajiUlang::find()
            ...
            ->andWhere([EvaluasiKinerja::tableName() . '.id_ref_dosen' => $dosen->id]);

        if (Yii::$app->assign->is(["administrator"]) || Yii::$app->assign->is(["admin dosen"])) {
            $query = EvaluasiKinerjaModel::find()
                ->where([static::tableName() . '.status' => 1]);
        } 
        // echo '<pre>';
        // print_r($query);
